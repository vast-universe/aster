...
export type SourceType = "official" | "github" | "http" | "namespace" | "local";
...
export function parseSource(input: string): ParsedSource {
  ...
  if (input.startsWith("http://") || input.startsWith("https://")) {
    ...
    const component = pathParts[pathParts.length - 1]?.replace(".json", "") || "unknown";
    return {
      type: "http",
      url: input,
      component,
  ...
  if (input.startsWith("./") || input.startsWith("/") || input.startsWith("~/")) {
    const component = input.split("/").pop()?.replace(".json", "") || "unknown";
    return {
      type: "local",
      filePath: input,
      component,
