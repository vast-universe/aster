...
import type { AsterConfig, InstalledResource, ResourceType } from "../types";
 ...
 * 读取配置
 */
export async function readConfig(cwd?: string): Promise<AsterConfig | null> {
  return readJson<AsterConfig>(getConfigPath(cwd));
}

/**
 * 写入配置
 */
export async function writeConfig(config: AsterConfig, cwd?: string): Promise<void> {
 ...
 * 确保配置存在
 */
export async function ensureConfig(cwd?: string): Promise<AsterConfig> {
 ...
 * 获取配置值
 */
export async function getConfigValue<K extends keyof AsterConfig>(
  key: K,
  cwd?: string
): Promise<AsterConfig[K] | undefined> {
  const config = await readConfig(cwd);
  return config?.[key];
}
