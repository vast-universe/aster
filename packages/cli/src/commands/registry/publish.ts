...
async function collectResources(distDir: string, type: ResourceType, index: any): Promise<any[]> {
  const resources: any[] = [];
...
export async function registryPublish(options: PublishOptions = {}): Promise<void> {
  ...
  if (!token) {
    logger.error("请先登录: npx aster login");
    return;
  }
  ...
  if (!(await fs.exists(indexPath))) {
    ...
    logger.dim("请先运行 npx aster registry build");
    return;
  }
  ...
  if (userInfo && !userInfo.namespaces.includes(namespace)) {
    ...
    logger.dim("运行 npx aster namespace create <name> 创建新命名空间");
    return;
  }
  ...
  try {
    ...
    for (const r of components) {
      logger.log(`  npx aster add @${namespace}/${r.name}`);
    }
    for (const r of hooks) {
      logger.log(`  npx aster add @${namespace}/hook:${r.name}`);
    }
    for (const r of lib) {
      logger.log(`  npx aster add @${namespace}/lib:${r.name}`);
    }
    for (const r of configs) {
      logger.log(`  npx aster add @${namespace}/config:${r.name}`);
    }
